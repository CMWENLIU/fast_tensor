...
def create_unistring():
  size = sum(1 for line in open('uniwords.uni'))
...
def get_all_words(embsize): #cut and get all words from files, then put to new files and all_words.all
  all_txt_files = glob.glob("*.txt")
  with open ('all_words.all', 'w', encoding = 'utf-8') as outfile:
    for f in all_txt_files:
      newfname = f + '.words'
      with open(f, 'r') as infile:
        with open(newfname, 'w', encoding = 'utf-8') as outwords:        
          for line in infile:
            count = 0
            for word in line.split():
              count += 1
...
def writemytxt(sen_length):
	all_txt_files = glob.glob("*.words")
...
if __name__== '__main__':
  workdir = '/home/sheep/bigdata/d7000'
  embed_size = 25
  sen_length = 280
  os.chdir(workdir)
  #get_all_words(embed_size)
  #get_uni_word()
  #create_unistring()
  #writemytxt(sen_length)	
  myGlove()	
	#replacechar()
	#roundnumbers()
	#remove_dup()
